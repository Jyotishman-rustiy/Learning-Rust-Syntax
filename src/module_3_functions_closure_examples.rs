...
pub fn multiple_return_example(){
    ...
    println!("Total Tokens {} . Gas Fee {} . Balance after Transaction {} ." , tokens, fee, balance)
...
pub fn higher_order_function_example(){
 ...
 fn apply_fee<F>(f: F, gas_limit: i32) -> i32
        ...
        F: Fn(i32) -> i32,
    {
        ...
        let total_fee:i32 = apply_fee(calculate_fee, 100);
        println!("Total fee bound on gas limit :{} ", total_fee);
...
pub fn closure_capture_examples(){
    let base_gas_price = 10;
    let calculate_total_cost = |gas_used: i32| gas_used * base_gas_price ;
    let total_cost = calculate_total_cost(5);

    println!(
        "Total transaction cost : {} (gas used * base gas price)", total_cost
    )
}


pub fn returning_closure_example(){
    fn create_staking_reward(multiplier:i32) -> impl Fn(i32) -> i32{
        move |tokens_staked| tokens_staked * multiplier
